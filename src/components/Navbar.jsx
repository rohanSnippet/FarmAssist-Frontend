...
const Navbar = () => {
  ...
  const IconLanguage = () => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
      <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 21l5.25-11.25L21 21m-9-3h7.5M3 5.621a48.474 48.474 0 016-.371m0 0c1.12 0 2.233.038 3.334.114M9 5.25V3m3.334 2.364C11.176 10.658 7.69 15.08 3 17.502m9.334-12.138c.896.298 1.744.605 2.534.935 3.194 1.332 5.093 3.398 4.414 5.922-.359 1.334-1.218 2.05-2.074 2.373" />
    </svg>
  );
  const IconTheme = () => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    ...
    <>
      ...
      >
        <div className="navbar-start">
          ...
          <Link to="/" className="btn btn-ghost text-xl flex items-center gap-2 px-1">
            <img src={isDark ? LogoLight : Logo} alt="FarmAssist" className="h-8 w-8" />
            <span className="hidden sm:inline text-lg font-bold">{t("navbar.logo_text")}</span>
          </Link>
        </div>
        ...
        <div className="navbar-center hidden lg:flex">
          <ul className="menu menu-horizontal px-1 gap-1">
            <li>
              <NavLink to="/" className={({ isActive }) => isActive ? "active font-bold text-primary bg-primary/10" : "hover:text-primary" }>
                {t("navbar.home")}
              </NavLink>
            </li>
            <li>
              <NavLink to="/about-us" className={({ isActive }) => isActive ? "active font-bold text-primary bg-primary/10" : "hover:text-primary" }>
                {t("navbar.about_us")}
              </NavLink>
            </li>
            {/* <li>
        ...
        <div className="navbar-end gap-2">
          ...
          {loading ? (
            ...
            <button
              onClick={() => {
                setSidebarOpen(true);
                setAlertOpen(false); // Close alert drawer to prevent overlap
              }}
            ...
            <>
              <Link to="/login" className="btn btn-primary btn-sm md:btn-md shadow-lg shadow-primary/20">
                {t("Common.login")}
              </Link>
              <button
      ...
      {/* ================= RIGHT SIDEBAR (PROFILE/SETTINGS DRAWER) ================= */}
      <div className={`fixed inset-0 z-50 transition-all duration-300 ${isSidebarOpen ? "visible" : "invisible"}`}>
        ...
        />
        <div
          className={`absolute right-0 top-0 h-full w-80 max-w-[90vw] bg-base-100 shadow-2xl transform transition-transform duration-300 flex flex-col border-l border-base-200 ${isSidebarOpen ? "translate-x-0" : "translate-x-full"}`}
        >
          {/* Profile Header */}
          <div className="p-6 bg-base-200/50 border-b border-base-200 relative">
            ...
            {userData ? (
              <div className="flex flex-col items-center mt-2">
                <div className="avatar mb-3">
                  <div className="w-24 rounded-full ring-2 ring-primary ring-offset-base-100 ring-offset-3">
                    <img
                      src={userData?.photo_url || auth?.currentUser?.photoURL || "https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp"}
                      alt="User"
                      referrerPolicy="no-referrer"
                    />
                  </div>
              ...
              <div className="text-center mt-8">
                <h3 className="font-bold text-lg mb-2">{t("navbar.welcome")}</h3>
                <p className="text-sm opacity-60 mb-4">{t("navbar.login_to_access")}</p>
          ...
          <div className="p-4 flex-1 overflow-y-auto">
            <p className="text-xs font-bold text-base-content/40 uppercase mb-3 tracking-wider px-1">{t("navbar.settings_tools")}</p>
            {userData && (
              <button 
                onClick={() => {
                  navigate('/pest-prediction');
                  setSidebarOpen(false); // Close the drawer
                }}
              ...
              >
                ...
                <span className="font-bold text-lg">{t("navbar.scan_crop_health")}</span>
              </button>
            )}
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={cycleTheme} className="btn h-auto py-4 flex flex-col gap-2 border-base-200 hover:border-primary hover:bg-base-200/50 normal-case">
                <IconTheme />
                <span className="text-xs font-bold">{t("navbar.theme")}</span>
              ...
              <button onClick={toggleFullScreen} className="btn h-auto py-4 flex flex-col gap-2 border-base-200 hover:border-primary hover:bg-base-200/50 normal-case col-span-2">
                <IconFullscreen />
                <span className="text-xs font-bold">{isFullscreen ? t("navbar.exit_fullscreen") : t("navbar.enter_fullscreen")}</span>
              </button>
            </div>
            {userData && (
              <div className="mt-6 ">
                <p className="text-xs font-bold text-base-content/40 uppercase mb-3 tracking-wider px-1">{t("navbar.account")}</p>
                <ul className="menu bg-base-200/30 rounded-box p-2 w-full">
                  <li><Link to="/my-farms" onClick={() => setSidebarOpen(false)}>{t("navbar.my_farms")}</Link></li>
                  <li><Link to="/history" onClick={() => setSidebarOpen(false)}>{t("navbar.saved_recommendations")}</Link></li>
                </ul>
              </div>
            )}
          </div>
          ...
          {userData && (
            <div className="p-4 border-t border-base-200 bg-base-100">
              <button onClick={handleLogout} className="btn btn-error btn-outline w-full gap-2">{t("Common.logout")}</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 ${isAlertOpen ? "visible" : "invisible"}`}>
        {/* Backdrop */}
        <div
        ...
        {/* Drawer Content */}
        <div
          className={`absolute right-0 top-0 h-full w-[400px] max-w-[90vw] bg-base-100 shadow-2xl transform transition-transform duration-300 flex flex-col border-l border-base-200 ${isAlertOpen ? "translate-x-0" : "translate-x-full"}`}
        >
          {/* Close Button overlaying the Inbox Header */}
          <button
            onClick={() => setAlertOpen(false)}
          ...
          {/* Render your AlertInbox here! (Make sure AlertInbox has h-full inside its own return) */}
          <AlertInbox />
        </div>
      </div>
